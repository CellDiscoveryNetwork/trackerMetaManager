...
def format_all_sheets(spreadsheet_id, credentials):
    ...
    while attempt < 8:  # Allow up to 10 attempts
        try:
            for sheet in sheets:
            ...
            break  # Exit loop after successful reformatting
        except Exception as e:
            if 'Quota exceeded' in str(e) and attempt < 7:  # Check if the error is due to quota exceeded
                sleep_time = backoff(attempt)
                print(f"Quota exceeded while formatting sheets, retrying after {sleep_time:.2f} seconds...")
                time.sleep(sleep_time)
                attempt += 1
            else:
                print(f"Failed to reformat sheet {spreadsheet_id} on attempt {attempt + 1}: {str(e)}")
    ...
    while attempt < 10:
        try:
            ...
            if 'Quota exceeded' in str(e) and attempt < 7:  # Check if the error is due to quota exceeded
                ...
                print(f"Failed to set dropdowns on attempt {attempt + 1}: {str(e)}")
                break  # Break after max attempts or other types of errors
